...
with st.container():
    ...
    with c1:
        if st.button("Save Student"):
            if filled:
                path = "D:\\p1 html\\Attendance-System-Project-main\\1. Dataset\\"+name+".jpg"
                ...
                df = pd.read_csv("Attendance-System-Project-main\\5. Encodings\\encodings.csv")
                en = df["Encodings"].tolist()
                n = df["Persons"].tolist()
                en.append(img_encoding)
                n.append(name)
                df = pd.DataFrame({"Persons": n, "Encodings": en})
                df.to_csv("Attendance-System-Project-main\\5. Encodings\\encodings.csv", index=False)
